<?php
    ...
    $resultset = null;
    try {
        $querystring = sprintf("SELECT b.DewarName
                                FROM DewarDefinition b
                                WHERE b.DewarName LIKE '%s' LIMIT 1;", $dewarname);
    ...
    if($resultset && $resultset->rowCount() != 0) {
        echoError($xmldoc, $key, "dewar with name '$dewarname' already exists");
        die();
    }
    
    $querystring = sprintf("INSERT INTO DewarDefinition (DewarName)
                            VALUES ('%s');",
                                    $dewarname);
    try {
        $resultset = $dbh->query($querystring);
